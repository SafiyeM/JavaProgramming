...
public class MapPractice2 {
    ...
    public static void main(String[] args) {
        ...
        // Object can be reference all data types
        ...
        // to get the value, we use key
        System.out.println(person2.get("name")); // Nora
        System.out.println(person2.get("job_title")); // Back-end Developer
        ...
        // if we have multiple map objects, to manage them efficiently we can create List of Map
        // ref type List                         object type-> accepts Map objects
            ...
            for (Map.Entry<String, Object> eachEntry : eachMap.entrySet()) {
                if (eachEntry.getKey().equals("hired_date")) {
                    if (((LocalDate) eachEntry.getValue()).getYear() == 2022) {
                        System.out.println(eachMap.get("name")); // Nora Iskender Umran
        ...
        //  gets last map object in the list of map and change the salary(key) with replace()
                                                                  // to get the original salary from this map,  we call get("salary")
        listOfMaps.get(listOfMaps.size() - 1).replace("salary", ((Integer) listOfMaps.get(listOfMaps.size() - 1).get("salary")) + 50000);
                                                                // get() method returns value and the value we have now 'Object'
                                                                // reference type is object not Integer, that's why we downcast it into the Integer
                                                                // in order to increase by 50_000


        listOfMaps.get(1).replace("age", ((Integer) listOfMaps.get(1).get("age")) + 4); // we update only one value
