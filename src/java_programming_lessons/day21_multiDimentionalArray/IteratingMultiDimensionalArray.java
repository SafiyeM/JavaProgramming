...
public class IteratingMultiDimensionalArray {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < arr2D.length; i++) { // i: index number of one dimensional Array in arr2D
            ...
            int[] each1D = arr2D[i]; // (arr2D[i]) -> gets each single dimensional array and assigned to each1D arrays' variable
        ...
        for (int i = arr2D.length - 1; i >= 0; i--) {        // index of 1 D array
            for (int j = arr2D[i].length - 1; j >= 0; j--) { // index of elements
