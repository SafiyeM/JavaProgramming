...
public class FrequencyOfWord {
    ...
    public static int frequencyOfWord(String sentence, String word){
        ...
        int startLength = sentence.length(); // we keep track of the original length to use it later to compare and with the frequency
        sentence = sentence.replace(word, ""); // this will replace every occurrence of the given word in the sentence

        return (startLength - sentence.length()) / word.length(); // we find the difference in the length before and after removing the word.
                                                                 // Then divide by how long the word is to determine how many times that word
        //                                                      //was removed from the sentence, which was the frequency of the word
    }
